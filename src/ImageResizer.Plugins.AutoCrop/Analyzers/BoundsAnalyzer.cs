...
namespace ImageResizer.Plugins.AutoCrop.Analyzers
{
    public class BoundsAnalyzer
    {
        ...
        public BoundsAnalyzer(BitmapData bitmap, int threshold)
        {
            ...
            {
                BoundingBox = GetBoundingBoxForContent(bitmap, borderAnalyzer.BackgroundColor, threshold);
                FoundBoundingBox = !imageBox.Equals(BoundingBox);
            }
        }

        private unsafe Rectangle GetBoundingBoxForContent(BitmapData bitmap, Color backgroundColor, int threshold)
        {
